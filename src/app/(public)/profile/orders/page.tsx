...
export default function OrderHistoryPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case 'SHIPPING':
                return <Badge variant="outline" className="bg-blue-50 text-[#082a5a] border-blue-200">กำลังจัดส่ง</Badge>;
            case 'COMPLETED':
            ...
            {orders.length === 0 ? (
                <Card className="text-center py-16 bg-slate-50 border-dashed">
                    <CardContent className="flex flex-col items-center gap-4">
                        ...
                        <Button className="mt-4 bg-[#0B3979] hover:bg-[#082a5a]" asChild>
                    ...
                    {orders.map((order) => (
                        <Card key={order.id} className="overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            <CardFooter className="bg-slate-50/30 border-t p-4 flex justify-end">
                                <Button variant="ghost" className="text-[#0B3979] hover:text-[#082a5a] hover:bg-blue-50" asChild>
